...
const mapItems = (
  ...
  items: DeploymentItems[] | TaskItems[],
  dot: JSX.Element
) => {
  if (type === "deployment") {
...
const InternalTimeline: React.ForwardRefRenderFunction<
  HTMLElement,
  LagoonTimelineProps
> = ({ items, type }, ref) => {
  const iconToUse =
    type === "deployment" ? <CloudUploadOutlined /> : <CarryOutOutlined />;
...
const LagoonTimeline = forwardRef<HTMLElement, LagoonTimelineProps>(
  InternalTimeline
);
