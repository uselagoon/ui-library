...
const InternalDataCard: React.ForwardRefRenderFunction<HTMLDivElement, CardProps> = (props, ref) => {
	const { text, count } = props;

	const keysToCheck = ['critical', 'high', 'medium', 'low'];

	const severityColor = keysToCheck.every((key) => !(key in props)) ? '#fff' : getColor(props as ConditionProps);
